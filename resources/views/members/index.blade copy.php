...
<body>
                            ...
                            <div>
                                ...
                                <div class="mt-3">
                                    <h6 class="text-dark fw-bold border-bottom pb-2">
                                        Daftar Cabang & Expired Date
                                        <button class="btn btn-sm btn-outline-primary float-end" onclick="loadBranches({{ $member->id }}, '{{ $member->link }}')">Refresh</button>
                                    </h6>
    ...
    <script>
        ...
        function loadBranches(memberId, baseUrl) {
            ...
            // Tampilkan loading
            container.innerHTML = '<div class="text-center text-muted py-2"><small>Memuat...</small></div>';

            // Hapus trailing slash jika ada pada link
            ...
            axios.get(`${cleanUrl}/api/get-setting-cabang`)
                .then(res => {
                    ...
                    if(branches.length === 0) {
                        html = '<div class="text-center text-muted"><small>Tidak ada data cabang</small></div>';
                    } else {
                        branches.forEach(branch => {
                            // Format tanggal YYYY-MM-DD untuk input date
                            // Data dari Laravel biasanya "2025-11-21T17:..." atau "2025-11-21"
                            let defaultDate = '';
                            if(branch.expired_date) {
                                defaultDate = branch.expired_date.split('T')[0];
                            }
                            ...
                            html += `
                                <div class="branch-item">
                                    <div class="d-flex justify-content-between align-items-center mb-1">
                                        ...
                                        <span class="badge bg-secondary">ID: ${branch.id}</span>
                                    </div>
                ...
                .catch(err => {
                    console.error(err);
                    container.innerHTML = `<div class="text-danger text-center"><small>Gagal koneksi ke Web 2.<br>${cleanUrl}</small></div>`;
                });
        }

        // 3. Fungsi Update Expired Date ke Web 2
        function saveBranchExpired(baseUrl, branchId, memberId) {
            ...
            if (!dateInput) {
                return Swal.fire('Peringatan', 'Tanggal harus diisi!', 'warning');
            }
            ...
            .catch(err => {
                console.error(err);
                Swal.fire('Error', 'Gagal menghubungi Web 2', 'error');
            })
            .finally(() => {
