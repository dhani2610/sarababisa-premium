...
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
                                ...
                                <div class="px-5 py-4">
                                    <div class="space-y-3">
                                        ...
                                        <div><label class="block text-sm font-medium mb-1" for="name">Nama Karyawan
                                                ...
                                                required /></div>
                                        <div><label class="block text-sm font-medium mb-1" for="jabatan">Jabatan <span
                                                    class="text-rose-500">*</span></label><input id="jabatan"
                                                name="jabatan" class="form-input w-full px-2 py-1" type="text"
                                                required /></div>
                                        <div>
                                            <label class="block text-sm font-medium mb-1" for="status">Status <span
                                        ...
                                        <div><label class="block text-sm font-medium mb-1" for="bulankerja">Bulan Kerja
                                        ...
                                        <div>
                                            ...
                                            <div class="relative"><input id="absen" name="absen"
                                                    class="form-input w-full pl-10 px-2 py-1 input-currency"
                                        ...
                                        <div>
                                            <label class="block text-sm font-medium mb-1" for="bpjs">BPJS <span
                                                    class="text-rose-500">*</span></label>
                                            <div class="relative"><input id="bpjs" name="bpjs"
                                                    class="form-input w-full pl-10 px-2 py-1 input-currency"
                                                    type="text" required placeholder="0" />
                                                <div
                                                    class="absolute inset-0 right-auto flex items-center pointer-events-none">
    ...
    <div class="bg-white shadow-lg rounded-sm border border-slate-200 mt-5 mb-8">
            ...
            <div class="relative inline-flex">
                <div class="table-items-action hidden">
                    <div class="flex items-center">
                        <div class="text-sm italic mr-2 whitespace-nowrap"><span class="table-items-count">0</span>
        ...
        <form class="flex flex-row gap-2 items-end w-full p-4">

            <div class="w-24 shrink-0">
                <label class="block text-sm font-medium mb-1 text-slate-600">Tahun</label>
                <select id="filter_year" class="form-select w-full text-sm py-2">
            ...
            <div class="flex-1 min-w-[120px]">
                <label class="block text-sm font-medium mb-1 text-slate-600">Bulan</label>
                <select id="filter_month" class="form-select w-full text-sm py-2">
            ...
            <div class="shrink-0">
                <button type="button" id="btn-filter"
                    class="btn bg-indigo-500 hover:bg-indigo-600 text-white py-2 px-4 whitespace-nowrap">
                    <span class="hidden sm:inline">Terapkan</span> <span class="sm:hidden">Go</span>
            ...
            <table id="karyawan-table" class="table-auto w-full">
                <thead
                    ...
                    <tr>
                        ...
                        <th class="px-2 py-3">Bonus</th>
                        <th class="px-2 py-3">Gaji Pokok</th>
                        <th class="px-2 py-3">Absen</th>
                        <th class="px-2 py-3">BPJS</th>
                        <th class="px-2 py-3">Kasbon</th>
                        <th class="px-2 py-3">Insiden</th>
...
<div id="printModalWrapper" class="relative z-50 hidden" aria-labelledby="modal-title" role="dialog"
    aria-modal="true">
        ...
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div
                ...
                <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                    <h3 class="text-base font-semibold leading-6 text-gray-900 mb-4" id="printModalTitle">Cetak Slip
                        Gaji</h3>
...
<script>
    ...
    $(document).ready(function() {
        ...
        // --- DATA TABLES INIT dengan Filter ---
        var table = $('#karyawan-table').DataTable({
            processing: true, // Ubah ke true agar loading terlihat saat filter
            serverSide: true, // Gunakan server-side rendering untuk data collection custom
            ajax: {
        ...
        // Trigger reload table saat filter berubah
       ...
       // --- MODIFIED PRINT MODAL FUNCTION ---
        // Menerima parameter tambahan 'periodeValue'
        function openPrintModal(id, name, hp, periodeValue) {
            $('#modal_worker_id').val(id);
            $('#modal_worker_name').val(name);
            $('#modal_worker_hp').val(hp);

            // Set Judul
            $('#printModalTitle').text('Cetak Slip Gaji: ' + name);

            // LOGIC OTOMATIS: Set Input Hidden Tanggal & Periode
            // 1. Tanggal cetak (Penanggalan) otomatis hari ini (sudah di set di value default HTML, tapi bisa di force disini)
            const today = new Date().toISOString().split('T')[0];
            $('#modal_penanggalan').val(today);
            console.log(periodeValue);
