<div>
    ...
    <div class="sm:flex sm:justify-between sm:items-center mb-3">
        ...
        <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
            ...
            <!-- Search form -->
            <x-search-form placeholder="Pelanggan/Nomor Servis/Tindakan/Barang/IMEI" />
                        ...
                        <form action="{{ route('admin-transaksi-servis.store') }}" method="post">
                            ...
                            <div class="px-5 py-4 border-t border-slate-200">
                                <div class="flex flex-wrap justify-between space-x-2">
                                    <a href="{{ route('admin-pelanggan.index') }}" class="btn-sm bg-green-500 hover:bg-green-600 text-white">
                ...
                <tbody class="text-sm divide-y divide-slate-200">
                    ...
                    @foreach($service_transactions as $transaction)
                        ...
                        <tr class="{{ $color }}">
                            ...
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-medium text-blue-600">{{ $transaction->nomor_servis }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    ...
                                    @if ($transaction->customer != null)
                                        ...
                                        >
                                            @if ($transaction->exp_garansi != null)
                                                <a href="https://wa.me/{{ $nomorwa }}/?text=*Notifikasi%20Service*%0A{{ $toko->nama_toko }}%0A%0ANo.%20Service%20:%20{{ $transaction->nomor_servis }}%0ANama%20user%20:%20*{{ $transaction->nama_pelanggan }}*%0AUnit%20:%20{{ $transaction->nama_barang }}%0ADiambil%20:%20{{ $transaction->pengambil }}%0ATanggal%20:%20{{ \Carbon\Carbon::parse($transaction->tgl_ambil)->translatedFormat('d F Y (H:i)') }}%0AStatus%20:%20{{ $transaction->kondisi_servis }}%0AGaransi%20sampai%20:%20{{ \Carbon\Carbon::parse($transaction->exp_garansi)->translatedFormat('d F Y') }}%0APembayaran%20:%20{{ $transaction->cara_pembayaran }}%0A%0ALink%20tracking%20:%20{{ env('APP_URL') }}/tracking%0A%0ATerimakasih" target="__blank">
                                            ...
                                            @else
                                                <a href="https://wa.me/{{ $nomorwa }}/?text=*Notifikasi%20Service*%0A{{ $toko->nama_toko }}%0A%0ANo.%20Service%20:%20{{ $transaction->nomor_servis }}%0ANama%20user%20:%20*{{ $transaction->nama_pelanggan }}*%0AUnit%20:%20{{ $transaction->nama_barang }}%0ADiambil%20:%20{{ $transaction->pengambil }}%0ATanggal%20:%20{{ \Carbon\Carbon::parse($transaction->tgl_ambil)->translatedFormat('d F Y (H:i)') }}%0AStatus%20:%20{{ $transaction->kondisi_servis }}%0AGaransi%20sampai%20:%20Tidak%20ada%20garansi%0APembayaran%20:%20{{ $transaction->cara_pembayaran }}%0A%0ALink%20tracking%20:%20{{ env('APP_URL') }}/tracking%0A%0ATerimakasih" target="__blank">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-file-invoice" width="20" height="20" viewBox="0 0 24 24" stroke-width="1.5" stroke="#00abfb" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                    <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                                                    <path d="M14 3v4a1 1 0 0 0 1 1h4" />
                                                    <path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2z" />
                                                ...
                                                <div class="bg-white rounded shadow-lg overflow-auto max-w-xl w-full max-h-full" x-on:keydown.escape.window="showPrint = false">
                                                    ...
                                                    <div class="px-5 py-4">
                                                        <div class="flex flex-wrap justify-end space-x-2">
                                                            <a x-bind:href="'{{ route('admin-termal-pengambilan', '') }}/' + printId" target="__blank">
                                                            ...
                                                            <a x-bind:href="'{{ route('admin-inkjet-pengambilan', '') }}/' + printId" target="__blank">
