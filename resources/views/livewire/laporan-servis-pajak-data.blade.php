<div>
            ...
            <div class="relative inline-flex" x-data="{ modalOpen: false }">
                ...
                <div id="tambah-modal"
                    ...
                    <div class="bg-white rounded shadow-lg overflow-auto max-w-lg w-full max-h-full"
                        ...
                        <!-- Modal content -->
                        <form action="{{ route('cetak-laporan-pajak-servis') }}" method="get"  hidetargetblank>
                            @csrf
                            <div class="px-5 py-4">
                                <div class="space-y-3">
                                    <div>
    ...
    <div class="bg-white shadow-lg rounded-sm border border-slate-200 mt-5 mb-8">
        <header class="px-5 py-4">
            <h2 class="font-semibold text-slate-800">Jumlah Servis Ditangani <span
                    class="text-slate-400 font-medium">{{ $jumlah }}</span></h2>
        </header>
        <!-- Table -->
            ...
            <table class="table-auto w-full">
                ...
                <thead
                    ...
                    <tr>
                        ...
                        <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                            <div class="font-semibold text-left">Grand Total</div>
                        </th>
                    </tr>
                </thead>
                <!-- Table body -->
                <tbody class="text-sm divide-y divide-slate-200">
                    ...
                    @foreach ($services as $item)
                        @php
                            ...
                            $totalPpn = $item->biaya + $pajak;

                            // $dasar = $item->biaya - $item->modal_sparepart - $item->diskon - $item->uang_muka;
                            // $pajak = 0;
                            // $ppn = $item->ppn;
                            // if (!empty($ppn) && $ppn != 0) {
                            //     $pajak = ($ppn / 100) * $dasar;
                            // }
                            // $totalPpn = $dasar + $pajak;
                        @endphp
                        <tr>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-medium">
                                    {{ \Carbon\Carbon::parse($item->tgl_disetujui)->translatedFormat('d F Y') }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-medium">
                                    {{ \Carbon\Carbon::parse($item->tgl_ambil)->translatedFormat('d F Y') }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                @if ($item->user)
