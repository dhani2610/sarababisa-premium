<div>
    ...
    <div class="sm:flex sm:justify-between sm:items-center mb-3">
        ...
        <div class="mb-4 sm:mb-0">
            <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">Hasil Target ✨</h1>
        </div>
        ...
        <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
            ...
            <!-- Search form -->
            <x-search-form placeholder="Cari berdasarkan tanggal" />
            ...
            <div x-data="{ modalOpen: false }">
                <button class="btn bg-indigo-500 hover:bg-indigo-600 text-white" @click.prevent="modalOpen = true" aria-controls="tambah-modal">
                    ...
                    <span class="hidden xs:block ml-2">Tambah Hasil Target</span>
                </button>
                <!-- Modal backdrop -->
                <div
                ...
                >
                    <div class="bg-white rounded shadow-lg overflow-auto max-w-lg w-full max-h-full" @click.outside="modalOpen = false" @keydown.escape.window="modalOpen = false">
                        ...
                        <div x-data="{ tab: '1' }" class="px-5 py-4">
                            ...
                            <!-- Item 2 -->
                            <div x-show="tab === '2'">
                                <form action="{{ route('target-bulan-sebelumnya') }}" method="post">
                                    @csrf
                                    <!-- Modal footer -->
                                    <div class="pt-4 border-t border-slate-200 mt-4">
                                        <div class="flex flex-wrap justify-end space-x-2">
    ...
    <div class="bg-white shadow-lg rounded-sm border border-slate-200 mt-5 mb-8">
        <div x-data="handleSelect">
            <div class="sm:flex sm:justify-between sm:items-center px-5 py-4">
                {{-- Left side --}}
                <h2 class="font-semibold text-slate-800">Semua Hasil Target <span class="text-slate-400 font-medium">{{ $targets_count }}</span></h2>
                {{-- Right side --}}
                    ...
                    <tbody class="text-sm divide-y divide-slate-200">
                        ...
                        @foreach($targets as $item)
                            <tr>
                                ...
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    <div class="font-medium">Rp. {{ number_format($item->target) }}</div>
                                </td>
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    <div class="font-medium">{{ $item->persen }}%</div>
                                </td>
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    <div class="font-medium">Rp. {{ number_format($item->nilai) }}</div>
                                </td>
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px">
