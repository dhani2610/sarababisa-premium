<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
                    ...
                    <div class="fixed inset-0 z-50 overflow-hidden flex items-center my-4 justify-center px-4 sm:px-6" role="dialog" aria-modal="true" x-show="modalOpen" x-transition:enter="transition ease-in-out duration-200" x-cloak>
                        <div class="bg-white rounded shadow-lg overflow-auto max-w-lg w-full max-h-full" @click.outside="modalOpen = false">
                            <div class="px-5 py-3 border-b border-slate-200 flex justify-between items-center">
                                <div class="font-semibold text-slate-800">Tambah Tipe OS</div>
                                <button class="text-slate-400 hover:text-slate-500" @click="modalOpen = false">&times;</button>
                            </div>
                            <form action="{{ route('master-tipe-os.store') }}" method="post">
                                @csrf
                                <div class="px-5 py-4">
                                    <div class="space-y-3">
                                        <div>
                                        ...
                                        <input name="nominal_bonus" type="hidden" value="0" />
                                    </div>
                                </div>
        ...
        <div class="bg-white shadow-lg rounded-sm border border-slate-200 mt-5 mb-8">
            ...
            <div class="sm:flex sm:justify-between sm:items-center px-5 py-4">
                <h2 class="font-semibold text-slate-800">Semua Tipe OS <span class="text-slate-400 font-medium">{{ $count }}</span></h2>
            ...
            <div class="overflow-x-auto p-4">
                <table id="tipe-os-table" class="table-auto w-full">
        ...
        <script>
            $(document).ready(function() {
                var table = $('#tipe-os-table').DataTable({
                    processing: false,
                    serverSide: false,
                    ajax: "{{ route('master-tipe-os.data') }}",
                    columns: [
                ...
                function attachCheckboxHandlers() {
                    ...
                    $('#tipe-os-table').off('change', '.table-item').on('change', '.table-item', function() {
                ...
                window.deleteSelected = function() {
                    ...
                    $.ajax({
                        url: "{{ route('master-tipe-os.delete-batch') }}",
                        method: 'POST',
                        data: {
                            ids: selectedIds,
                            _token: "{{ csrf_token() }}"
