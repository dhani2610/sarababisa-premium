    ...
    <script>
        $(document).ready(function () {
            ...
            const sparepartOptions = `
                ...
                {{-- Addslashes agar aman dari karakter petik/garis miring --}}
            ...
            function generateActionHtml(groupIndex, actionIndex) {
                ...
                <div class="action-item" x-data="{ useSparepart: false, showInputManual: false }">
                    <button type="button" class="position-button-x remove-action absolute top-2 right-2 text-rose-500 hover:text-rose-700 font-bold" title="Hapus Tindakan Ini">&times;</button>
                    ...
                    <div class="konfirmasi-stok border-t border-slate-200 pt-2 mt-2">
                        ...
                        <div class="flex flex-wrap items-center -m-3 mb-2">
                            ...
                            <div class="m-3">
                                <label class="flex items-center">
                                    {{-- Tambahkan class khusus radio-sparepart-yes untuk trigger JS --}}
            ...
            function addTechnicianGroup(techData = null) {
                ...
                <div class="technician-group relative" data-group-index="${currentGroupIndex}">
                    ...
                    <div class="mt-3 text-center border-t border-dashed border-slate-300 pt-3">
                        <button type="button" class="add-action-btn btn-sm bg-emerald-500 hover:bg-emerald-600 text-white">+ Tambah Tindakan Lain (Untuk Teknisi Ini)</button>
                    </div>
                </div>`;
            ...
            function addActionToGroup($groupElement, groupIndex, actionData = null) {
                ...
                if (actionData) {
                    ...
                    // --- LOGIKA SPAREPART (EDITED) ---
                    if (actionData.prod_id && actionData.prod_id != "null" && actionData.prod_id != "") {

                        // 1. TRIGGER CLICK (Penting!)
                        // Kita tidak pakai .prop('checked', true). Kita pakai .click()
                        // Ini akan memicu AlpineJS untuk mengubah state x-show jadi true.
                        setTimeout(function(){
                            // Trigger click native JS element agar Alpine mendeteksi
                            $newItem.find('.radio-sparepart-yes')[0].click();
                        }, 50);

                        // 2. Set Value Select2
                        // Kita delay sedikit untuk memastikan div sudah dirender oleh Alpine/Browser
                        setTimeout(function() {
                            $selSparepart.val(actionData.prod_id).trigger('change');
                        }, 100);

                        // Jika mau bypass Alpine total (Force Show):
                        // $newItem.find('.wrapper-sparepart-area').show();
                        // Tapi .click() lebih aman agar sync dengan Alpine.
                    }
                }
            }
