...
<body>
  ...
  <h5 style="margin-top: 15px; margin-bottom: 6px;">Produk yang dijual :</h5>
  ...
  <h5 style="margin-top: 15px; margin-bottom: 6px;">Produk yang ditukar :</h5>
  ...
  <table width="100%" style="border: 1px solid; border-collapse: collapse;">
    ...
    <tbody>
     @foreach($produkTukarTambah as $item)
      <tr class="font">
        <td align="center" style="text-transform: uppercase;">
          {{ $item->product->product_name }} {{ $item->product->kondisi }} {{ $item->product->warna }} {{ $item->product->ram }}/{{ $item->product->capacity->name }} (IMEI {{ $item->product->nomor_seri }})
        </td>
        <td align="center">Rp. {{ number_format($item->product_price) }}</td>
      </tr>
      @endforeach
    </tbody>
  </table>
    ...
    <tr>
      ...
      <td align="right" >
          <h3 style="margin-bottom: 0;">
            Jumlah Pembayaran: Rp. {{ number_format($total - $hargaTukar) }} <br> <span style="text-transform: capitalize; font-size: 12px; font-weight: normal;">( {{ terbilang($total - $hargaTukar) }} rupiah )</span>
          </h3>
      </td>
    </tr>
    <tr>
