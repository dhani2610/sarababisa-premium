...
<x-toko-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-5">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">Pengeluaran Insiden ✨</h1>
            </div>

            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">

                <!-- Backup & Kelola Data Modal (Khusus Kepala Toko) -->
                <x-kepalatoko.data-management-modal module="insiden" />
                ...
                <div x-data="{ modalOpen: false }">
                    <button class="btn bg-indigo-500 hover:bg-indigo-600 text-white" @click.prevent="modalOpen = true">
                        ...
                        <span class="hidden xs:block ml-2">Tambah Insiden Baru</span>
                    </button>
                    ...
                    <div class="fixed inset-0 z-50 overflow-hidden flex items-center my-4 justify-center px-4 sm:px-6" role="dialog" aria-modal="true" x-show="modalOpen" x-transition:enter="transition ease-in-out duration-200" x-cloak>
                        <div class="bg-white rounded shadow-lg overflow-auto max-w-lg w-full max-h-full" @click.outside="modalOpen = false">
                            <div class="px-5 py-3 border-b border-slate-200 flex justify-between items-center">
                                <div class="font-semibold text-slate-800">Tambah Insiden</div>
                            ...
                            <form action="{{ route('insiden.store') }}" method="post">
                                ...
                                <div class="px-5 py-4">
                                    <div class="space-y-3">
                                        <div>
                                            <label class="block text-sm font-medium mb-1" for="name">Nama Insiden <span class="text-rose-500">*</span></label>
                                        ...
                                        <div>
                                            <label class="block text-sm font-medium mb-1" for="persen_teknisi">Persen beban teknisi (%) <span class="text-rose-500">*</span></label>
                                            <input id="persen_teknisi" name="persen_teknisi" class="form-input w-full px-2 py-1" type="number" required placeholder="Contoh: 50"/>
                                        </div>
                                    </div>
                                </div>
        ...
        <div class="bg-white shadow-lg rounded-sm border border-slate-200 mt-5 mb-8">
            <div class="sm:flex sm:justify-between sm:items-center px-5 py-4">
                <h2 class="font-semibold text-slate-800">Daftar Insiden <span class="text-slate-400 font-medium">{{ $incidents_count }}</span></h2>
            ...
            <div class="overflow-x-auto p-4">
                <table id="insiden-table" class="table-auto w-full">
                    <thead class="text-xs font-semibold uppercase text-slate-500 bg-slate-50 border-t border-b border-slate-200">
                        <tr>
                            ...
                            <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">Nama Insiden</th>
                            <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">Biaya Total</th>
                            <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">Teknisi</th>
                            <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">% Beban</th>
                            <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">Biaya Teknisi</th>
                            <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">Biaya Toko</th>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // 3. DataTables Init
            var table = $('#insiden-table').DataTable({
                processing: false,
                serverSide: false,
                ajax: "{{ route('insiden.data') }}",
                columns: [
                    ...
                    { data: 'persen_teknisi', name: 'persen_teknisi', render: function(data){ return data + '%'; } },
                    { data: 'biaya_teknisi', name: 'biaya_teknisi' },
                    { data: 'biaya_toko', name: 'biaya_toko' },
            ...
            function attachCheckboxHandlers() {
                ...
                $('#insiden-table').off('change', '.table-item').on('change', '.table-item', function() {
            ...
            window.deleteSelected = function() {
                ...
                $.ajax({
                    url: "{{ route('insiden.delete-batch') }}",
                    method: 'POST',
                    data: {
                        ids: selectedIds,
                        _token: "{{ csrf_token() }}"
