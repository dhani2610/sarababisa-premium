    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
            ...
            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                ...
                <div x-data="{ modalOpen: false }">
                    <button class="btn bg-indigo-500 hover:bg-indigo-600 text-white" @click.prevent="modalOpen = true">
                        ...
                        <span class="hidden xs:block ml-2">Tambah Kapasitas</span>
                    </button>
                    ...
                    <div class="fixed inset-0 z-50 overflow-hidden flex items-center my-4 justify-center px-4 sm:px-6" role="dialog" aria-modal="true" x-show="modalOpen" x-transition:enter="transition ease-in-out duration-200" x-cloak>
                        <div class="bg-white rounded shadow-lg overflow-auto max-w-lg w-full max-h-full" @click.outside="modalOpen = false">
                            <div class="px-5 py-3 border-b border-slate-200 flex justify-between items-center">
                                <div class="font-semibold text-slate-800">Tambah Kapasitas</div>
                                <button class="text-slate-400 hover:text-slate-500" @click="modalOpen = false">&times;</button>
                            </div>
                            <form action="{{ route('master-kapasitas.store') }}" method="post">
                                @csrf
                                <div class="px-5 py-4">
                                    <div class="space-y-3">
                                        <div>
                                            <label class="block text-sm font-medium mb-1" for="name">Nama Kapasitas <span class="text-rose-500">*</span></label>
        ...
        <div class="bg-white shadow-lg rounded-sm border border-slate-200 mt-5 mb-8">
            ...
            <div class="sm:flex sm:justify-between sm:items-center px-5 py-4">
                <h2 class="font-semibold text-slate-800">Semua Kapasitas <span class="text-slate-400 font-medium">{{ $capacities_count }}</span></h2>
            ...
            <div class="overflow-x-auto p-4">
                <table id="kapasitas-table" class="table-auto w-full">
                    <thead class="text-xs font-semibold uppercase text-slate-500 bg-slate-50 border-t border-b border-slate-200">
                        <tr>
                            ...
                            <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">Nama Kapasitas</th>
    ...
    @push('scripts')
        ...
        <script>
            $(document).ready(function() {
                var table = $('#kapasitas-table').DataTable({
                    processing: false,
                    serverSide: false,
                    ajax: "{{ route('master-kapasitas.data') }}",
                    columns: [
                ...
                function attachCheckboxHandlers() {
                    ...
                    $('#kapasitas-table').off('change', '.table-item').on('change', '.table-item', function() {
                ...
                window.deleteSelected = function() {
                    ...
                    $.ajax({
                        url: "{{ route('master-kapasitas.delete-batch') }}",
                        method: 'POST',
                        data: {
                            ids: selectedIds,
                            _token: "{{ csrf_token() }}"
